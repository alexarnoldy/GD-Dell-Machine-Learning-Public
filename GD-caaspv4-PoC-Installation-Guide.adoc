...
----
sudo bash -c "echo '${USERNAME} ALL=(ALL) NOPASSWD: ALL' > /etc/sudoers.d/01${USERNAME}"; exit
----
...
* Set the cluster nodes' fully qualified domain name, i.e. suse.hpc.local: `export DOMAIN=`
----
for EACH in `cat ~/.all_nodes`; do echo $EACH; ssh $EACH ping -c 2 ${MASTER_PREFIX}.${DOMAIN}; echo ""; done
----
