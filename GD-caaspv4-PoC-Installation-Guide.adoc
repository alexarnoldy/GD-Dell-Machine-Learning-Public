...
NOTE: Many of the steps in this process begin with running a short-lived ssh-agent process. While there's no harm in having multiple ssh-agents running at the same time, there's a security advantage to limiting any ssh-agent's lifetime. This methodology offers the added advantage of ensuring the ssh-agent is running during the procedure when it is required.
