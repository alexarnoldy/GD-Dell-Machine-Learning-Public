...
----
export USERNAME="geeko"
echo "${USERNAME} ALL=(ALL) NOPASSWD: ALL" > /tmp/01${USERNAME}
sudo chown root:root  /tmp/01${USERNAME}
sudo mv /tmp/01${USERNAME} /etc/sudoers.d/
----
* `ssh-keygen`
...
* Set the master nodes' hostname prefix, i.e. master or mstr: `export MASTER_PREFIX=`
* Set the worker nodes' hostname prefix, i.e. worker or wrkr: `export WORKER_PREFIX=`
* Set the cluster nodes' fully qualified domain name, i.e. suse.hpc.local: `export DOMAIN=`
----
cat <<EOF> ~/autoyast_templates/.all_nodes
${MASTER_PREFIX}1.${DOMAIN}
${MASTER_PREFIX}2.${DOMAIN}
${MASTER_PREFIX}3.${DOMAIN}
${WORKER_PREFIX}1.${DOMAIN}
${WORKER_PREFIX}2.${DOMAIN}
${WORKER_PREFIX}3.${DOMAIN}
${WORKER_PREFIX}4.${DOMAIN}
EOF

cp -p ~/autoyast_templates/.all_nodes ~/
----
...
* Launch nginx webserver container: 
* Set this variable to the local user: `export USERNAME="geeko"`
----
cd ~ 
sudo podman run --name autoyast-nginx -v /home/${USERNAME}/autoyast_templates:/usr/share/nginx/html:ro -p 80:80 -d nginx:latest
----
...
* Test that files in the autoyast_templates directory are available on the cluster network (if possible, from another system connected to the clsuter network): 
** Set this variable to the hostname (not the FQDN) of the Admin Node, aka Management Workstation (i.e. admin): `export ADMIN_NODE=""`
----
curl http://${ADMIN_NODE}/.all_nodes`
----
...
* From the Management Workstation, add the geeko@kubeadmin SSH credentials to each server: 
----
export USERNAME="geeko"
export HOSTNAME=""
ssh-copy-id -i /home/${USERNAME}/.ssh/id_rsa.pub ${HOSTNAME}
----

NOTE: SSH to, and peform the following steps on each of the two installed servers
...
* `export USERNAME="geeko"`
* `export ADMIN_NODE="kubeadmin.suse.hpc.local"`
----
sudo mv /root/autoinst.xml ~/$(hostname).xml
sudo chown -R ${USERNAME} ~/$(hostname).xml
scp ~/$(hostname).xml ${ADMIN_NODE}:~/autoyast_templates/
----
...
** An example of this operation for two additional master and three additional worker nodes, might be:
* Set the master nodes' hostname prefix, i.e. master or mstr: `export MASTER_PREFIX=`
* Set the worker nodes' hostname prefix, i.e. worker or wrkr: `export WORKER_PREFIX=`
----
cd ~/autoyast_templates/
for NODE in ${MASTER_PREFIX}2 ${MASTER_PREFIX}3; do cp -p ${MASTER_PREFIX}1.xml ${NODE}.xml; done
# Note: If all nodes are the same type of system and only the master1 node was clones, copy the master1.xml file to worker1.xml before running the following command
for NODE in ${WORKER_PREFIX}2 ${WORKER_PREFIX}3 ${WORKER_PREFIX}4; do cp -p ${WORKER_PREFIX}1.xml ${NODE}.xml; done
----
...
** Set this variable to the hostname (not the FQDN) of the installed server (i.e. master1 or worker1): `export FIRST_HOSTNAME=`
** Set the master nodes' hostname prefix, i.e. master or mstr: `export MASTER_PREFIX=`
** Set the worker nodes' hostname prefix, i.e. worker or wrkr: `export WORKER_PREFIX=`
** Execute this loop:
----
...
for EACH in $(ls -1 ${MASTER_PREFIX}* ${WORKER_PREFIX}*)
do 
NEW_HOSTNAME=$(echo ${EACH} | awk -F. '{print$1}')
echo "sed -i 's/${FIRST_HOSTNAME}/${NEW_HOSTNAME}/g' ${EACH}" >> /tmp/hostname_update
done
----
** Review the /tmp/hostname_update file to ensure the sed commands to change the hostnames are correct: `cat /tmp/hostname_update`
...
** If not already set, set the master nodes' hostname prefix, i.e. master or mstr: `export MASTER_PREFIX=`
** If not already set, set the worker nodes' hostname prefix, i.e. worker or wrkr: `export WORKER_PREFIX=`
** Execute this loop:
----
cd ~/autoyast_templates
rm -f /tmp/IP_update 
...
----
** Review the /tmp/hostname_update file to ensure the sed commands to change the IP addresses are correct: `cat /tmp/IP_update`
*** Correct any mistakes in the file, then execute the commands in it: `bash /tmp/IP_update`

.Ensure ipv4_forwarding is set to true in the XML files:
* Execute these commands:
----
cd ~/autoyast_templates
sed  -i '/ipv4_forward/ s/false/true/' *.xml 
----
...
do 
echo ${EACH}
curl -s http://${ADMIN_NODE}/${EACH}.xml| egrep "<hostname|ipaddr" | grep -v 127
echo ""
done
----
...
NOTE: If the webserver on the Managment Workstation is using a port other than 80, specify that port in the command below, i.e. `autoyast=http://<admin node>:<port>/<node_name>.xml`. The port can be verified by running `sudo podman ps` on the Management Workstation.

*** AutoYaST parameters: `autoyast=http://<admin node>/<node name>.xml ifcfg=em1=dhcp`
...
----
for EACH in `cat ~/.all_nodes`; do ssh $EACH ; done
----
* Execute this command for each node: 
----
sudo bash -c "echo '${USER} ALL=(ALL) NOPASSWD: ALL' > /etc/sudoers.d/01${USER}"; exit
----
...
*** If any hosts prompt for a password, resolve the issue with `ssh-copy-id -i /home/${USER}/.ssh/id_rsa.pub <hostname>` and retest

.Ensure IPv4 forwarding is not configured in the /etc/sysctl.conf file

NOTE: The skuba installation tool will configure IPv4 forwarding during installation. Pre-existing configurations can cause unexpected results.

* Run the following loop to comment out any IPv4 forwarding settings on the cluster nodes:
----
for EACH in $(cat .all_nodes); do echo ${EACH}; ssh ${EACH} sudo sed -i 's/^net.ipv4.ip_forward/#net.ipv4.ip_forward/' /etc/sysctl.conf; done
----
...
----
for EACH in `cat ~/.all_nodes`; do echo $EACH; ssh $EACH grep -v Filename /proc/swaps; echo ""; done
----
...
* Set this variable to the FQDN of the load balancer, i.e. mstr.suse.hpc.local: `export LB_FQDN=`
----
for EACH in `cat ~/.all_nodes`; do echo $EACH; ssh $EACH ping -c 2 ${LB_FQDN}; echo ""; done
----

==== Ensure all cluster nodes can reach resolve the name of, and reach, the RMT server

* Set the name of the RMT server, i.e. rmt: `export RMT_SHORTNAME=`
* Set the cluster nodes' fully qualified domain name, i.e. suse.hpc.local: `export DOMAIN=`
----
for EACH in `cat ~/.all_nodes`; do echo $EACH; ssh $EACH ping -c 2 ${RMT_SHORTNAME}.${DOMAIN}; echo ""; done
----
...
* Set the `CONTROL_PLANE` (set to the load balancer, mstr.suse.hpc.local in this guide) and `CLUSTER_NAME` (set to suse-caasp-hpc in this guide) variables and install the CaaS Platform management software and initialize the cluster definition:
----
...
----
export FIRST_MASTER_FQDN=$(grep ${MASTER_PREFIX} ~/.all_nodes | head -1)
export FIRST_MASTER=$(echo ${FIRST_MASTER_FQDN} | awk -F. '{print$1}')
----
----
eval "$(ssh-agent -t 180)"
ssh-add /home/${USER}/.ssh/id_rsa
skuba node bootstrap --user ${USER} --sudo --target ${FIRST_MASTER_FQDN} ${FIRST_MASTER}
kill ${SSH_AGENT_PID}
----
...
* To join all remainging Master Nodes in the ~/.all_nodes file, cd into the cluster directory (i.e. ~/suse-hpc-local/), then:
* Set the master nodes' hostname prefix, i.e. master or mstr: `export MASTER_PREFIX=`

----
## Start by gathering all of the master nodes, less the first one
for MASTER_FQDN in `grep ${MASTER_PREFIX} ~/.all_nodes | tail -n+2`; do \
eval "$(ssh-agent -t 180)"
ssh-add /home/${USER}/.ssh/id_rsa
...
* To join all remainging Worker Nodes in the ~/.all_nodes file, cd into the cluster directory (i.e. ~/suse-hpc-local/), then:
* Set the worker nodes' hostname prefix, i.e. worker or wrkr: `export WORKER_PREFIX=`

----
for WORKER_FQDN in `grep ${WORKER_PREFIX} ~/.all_nodes`; do \
eval "$(ssh-agent -t 180)"
ssh-add /home/${USER}/.ssh/id_rsa
...
* `cd /home/${USER}/suse-caasp-hpc`
* `skuba cluster status`

==== Enable the use of kubectl from the Management Workstation
* `echo export KUBECONFIG=/home/${USER}/suse-caasp-hpc/admin.conf >> ~/.bashrc`
* `. ~/.bashrc` 
* `kubectl get nodes`
...
.From the *CaaS Platform Administrative Workstation*, validate the nodes that are equipped with Nvidia GPUs
----
for EACH in `cat ~/.all_nodes`; do echo -n $EACH": "; ssh $EACH sudo lspci | grep -i nvidia; echo ""; done
----
...
* Verfity that you can access the GPU:
----
...
* Install the Nvidia libnvidia-container1:
...
* (Optional) Install Podman on the Worker Node and verify that a container can access the GPU: 
----
sudo zypper --non-interactive install podman podman-cni-config
...
* Set this variable to the first GPU equipped worker node's CaaSP node name (not the FQDN) for the next several commands, then repeat for each GPU equipped worker node: `export WORKER=`
...
.Ensure that CaaS Platform correctly schedules a pod needing GPUs on the node:
