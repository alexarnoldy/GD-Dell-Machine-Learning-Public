...
#### Last updated: 2019.12.23, early AM PDT
...
==== Setup podman and the nginx webserver container

* `sudo zypper -n in podman`
* Verify that port 80 on this host is not currently in use: `ss -npr --listening | grep :80`
** If port 80 is in use, specify a different port with the `-p <container port>:<host port>`` option, or a random high port with the `-P` option, when running the container
* Launch nginx webserver container: `sudo podman run --name autoyast-nginx -v /home/geeko/autoyast_templates:/usr/share/nginx/html:ro -p 80:80 -d nginx:latest`
...
NOTE: If the weberver is using a port other than 80, specify that port in the curl command below, i.e. `curl http://admin:<port>/.all_nodes`. The port can be verified with `sudo podman ps`

* Test that files in the autoyast_templates directory are available: `curl http://admin/.all_nodes`
** The output should display the contents of the .all_nodes file

==== Optionally, setup a local DNS/DHCP container

CAUTION: This step makes the process of AutoYaST installing the cluster nodes easier, but is not required. It is highly recommended to verify there is not an existing DHCP server on the subnet before continuing (See the Troubleshooting section below)

IMPORTANT: This step is primarily intended to support AutoYaST installation. The DNS/DHCP container can be used to support the installed and configured CaaS Platform cluster or the installed cluster hosts can refer to another DNS server.

* After cloning the repository in the link below, update the dnsmasq_hosts file with at least one entry that points to the Management Workstation, such as `172.16.240.130    admin`
* Ensure the DHCP configuration in the dnsmasq.conf file includes the correct default router and points to the container's IP address for DNS resolution

* Follow the README.adoc file to create a https://github.com/alexarnoldy/opensuse-dnsmasq-container[opensuse-dnsmasq-container]
...
NOTE: At this point the ~/autoyast_templates/ directory should have one XML file for each CaaS Platform cluster node, but all files are currently identical. The next steps will adjust those files to match the cluster nodes.

IMPORTANT: Ensure each cluster node has an XML file as well as an entry in the .all_nodes file

* Update the hostnames in the XML files:
...
----
** Review the /tmp/hostname_update file to ensure the changes are correct: `grep -v suse /tmp/hostname_update`
...
----
** Review the /tmp/hostname_update file to ensure the changes are correct: `grep -v suse /tmp/IP_update`
