...
* Set this variable for the next several commands: `WORKER=`
...
* If there are more than one node in the CaaS Platform cluster with GPUs, label this node to ensure the test pod get scheduled on it: `kubectl label node $WORKER gpu=true`
