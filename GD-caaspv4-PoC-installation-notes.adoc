...
*** AutoYaST parameters: `ifcfg=eth0=172.29.145.61/24,172.29.145.2,172.29.202.10,suse.hpc.local hostname=mstr1.suse.hpc.local`
** Format of ifcfg=eth0= is <node IP address/cidr mask>,<default gateway>,<primary DNS server>,<search domain>
* Press Enter
* Register with the RMT server and select the SUSE CaaS Platform 4.0 module (May need to uncheck "Hide Development Modules")
* For partitioning, select Guided Setup
...
NOTE: This document demonstrates the procdure for creating a bonded network from em1
